...
inline ethash::hash512 fnv1(const ethash::hash512& u, const ethash::hash512& v) noexcept
{
    ethash::hash512 r;
...
inline ethash::hash512 bitwise_xor(const ethash::hash512& x, const ethash::hash512& y) noexcept
{
    ethash::hash512 z;
...
int find_epoch_number(const ethash::hash256& seed) noexcept
{
    ...
    static thread_local ethash::hash256 cached_seed = {};
...
{
    ethash::hash512 item = hash_fn(seed.bytes, sizeof(seed));
    cache[0] = item;
    ...
    for (int q = 0; q < light_cache_rounds; ++q)
    {
        for (int i = 0; i < num_items; ++i)
        {
            ...
            const uint32_t t = ethash::le::uint32(cache[i].word32s[0]);
            const uint32_t v = t % index_limit;

            // Second index.
            const uint32_t w = static_cast<uint32_t>(num_items + (i - 1)) % index_limit;

            const ethash::hash512 x = bitwise_xor(cache[v], cache[w]);
    ...
    build_light_cache_fn build_fn, int epoch_number, bool full) noexcept
{
    ...
    ethash::hash512* const light_cache = reinterpret_cast<ethash::hash512*>(alloc_data + context_alloc_size);
    const ethash::hash256 epoch_seed = calculate_epoch_seed(epoch_number);
    ...
    ethash::hash1024* full_dataset = full ? reinterpret_cast<ethash::hash1024*>(l1_cache) : nullptr;
...
{
    return generic::build_light_cache(ethash::keccak512, cache, num_items, seed);
}

struct item_state
{
    const ethash::hash512* const cache;
        ...
        seed{static_cast<uint32_t>(index)}
    {
        ...
        mix.word32s[0] ^= ethash::le::uint32(seed);
        mix = ethash::le::uint32s(ethash::keccak512(mix));
    }
    ...
    ALWAYS_INLINE void update(uint32_t round) noexcept
    {
        ...
        mix = fnv1(mix, ethash::le::uint32s(cache[parent_index]));
    }

    ALWAYS_INLINE ethash::hash512 final() noexcept { return ethash::keccak512(ethash::le::uint32s(mix)); }
};
...
ethash::hash1024 calculate_dataset_item_1024(const epoch_context& context, uint32_t index) noexcept
{
    ...
    return ethash::hash1024{{item0.final(), item1.final()}};
}
...
ethash::hash2048 calculate_dataset_item_2048(const epoch_context& context, uint32_t index) noexcept
{
    ...
    return ethash::hash2048{{item0.final(), item1.final(), item2.final(), item3.final()}};
}

namespace
{
using lookup_fn = ethash::hash1024 (*)(const epoch_context&, uint32_t);

inline ethash::hash512 hash_seed(const ethash::hash256& header_hash, uint64_t nonce) noexcept
{
...
inline ethash::hash256 hash_final(const ethash::hash512& seed, const ethash::hash256& mix_hash)
{
...
inline ethash::hash256 hash_kernel(
    const epoch_context& context, const ethash::hash512& seed, lookup_fn lookup) noexcept
{
    static constexpr size_t num_words = sizeof(ethash::hash1024) / sizeof(uint32_t);
    ...
    ethash::hash1024 mix{{ethash::le::uint32s(seed), ethash::le::uint32s(seed)}};

    for (uint32_t i = 0; i < num_dataset_accesses; ++i)
    {
        const uint32_t p = fnv1(i ^ seed_init, mix.word32s[i % num_words]) % index_limit;
        const ethash::hash1024 newdata = ethash::le::uint32s(lookup(context, p));
    ...
    return ethash::le::uint32s(mix_hash);
}
}  // namespace
...
result hash(const epoch_context_full& context, const ethash::hash256& header_hash, uint64_t nonce) noexcept
{
    static const auto lazy_lookup = [](const epoch_context& ctx, uint32_t index) noexcept
    {
        ...
        ethash::hash1024& item = full_dataset[index];
        if (item.word64s[0] == 0)
        {
    ...
    const ethash::hash256 mix_hash = hash_kernel(context, seed, lazy_lookup);
...
search_result search_light(const epoch_context& context, const ethash::hash256& header_hash,
    const ethash::hash256& boundary, uint64_t start_nonce, size_t iterations) noexcept
{
...
search_result search(const epoch_context_full& context, const ethash::hash256& header_hash,
    const ethash::hash256& boundary, uint64_t start_nonce, size_t iterations) noexcept
{
    const uint64_t end_nonce = start_nonce + iterations;
    for (uint64_t nonce = start_nonce; nonce < end_nonce; ++nonce)
...
{
    const ethash::hash512 seed = hash_seed(*header_hash, nonce);
    const ethash::hash256 mix_hash = hash_kernel(*context, seed, calculate_dataset_item_1024);
    return {hash_final(seed, mix_hash), mix_hash};
}
    ...
    const ethash::hash256* mix_hash, uint64_t nonce, const ethash::hash256* boundary) noexcept
{
    ...
    const ethash::hash256 expected_mix_hash = hash_kernel(*context, seed, calculate_dataset_item_1024);
